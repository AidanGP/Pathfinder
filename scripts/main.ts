const CELL_COLOR : string = "rgb(229, 229, 229)";
const BLOCK_COLOR : string = "rgb(0, 0, 0)";
const NODE_COLOR : string = "rgb(252, 163, 17)";
const PATH_COLOR : string = "rgb(255, 255, 255)";

const VISUAL_COLORS : string[] = [
  "rgb(116, 0, 184)  ",
  "rgb(105, 48, 195) ",
  "rgb(94, 96, 206)  ",
  "rgb(83, 144, 217) ",
...
const SIZE_X : number = Math.floor($(window).width() / 50);
const SIZE_Y : number = Math.floor($(window).height() / 50);

function element(elementID : string) {
...
function setGrid() {
  ...
  let tableDiv : HTMLElement = element('dynamic_table');

  let table_grid = <HTMLTableElement> document.createElement('TABLE');
  let tableBody = <HTMLTableElement> document.createElement('TBODY');
  ...
  for (let table_row = 0; table_row < SIZE_Y; table_row++) {
    let table_row_element = <HTMLTableRowElement> document.createElement('TR');
    tableBody.appendChild(table_row_element);

    for (let table_column = 0; table_column < SIZE_X; table_column++) {
      const table_cell_ID: string = table_row + ',' + table_column;
      let table_cell = <HTMLTableCellElement> document.createElement('TD');

      let table_cell_label = <Text> document.createTextNode(table_cell_ID);
...
function setGridCells() {
  let table_grid = <HTMLTableElement>element('cell_grid');
  for (let row = 0; row < table_grid.rows.length; row++) {
    for (let col = 0; col < table_grid.rows[row].cells.length; col++) {
      const table_cell = <HTMLTableCellElement>table_grid.rows[row].cells[col];
      ...
      table_cell.onmousemove = function (event) {
        const mouse_btn: number = event.which;
...
async function plot_path(path: string[], color : string) {
	let table = <HTMLTableElement> element('cell_grid');

	for (var index = 0; index < path.length; index++) {
    var cell_index = parseInt(path[index]);
    const row : number = Math.floor(cell_index / SIZE_X);
    const column : number = cell_index - (SIZE_X * row);
		var cell = <HTMLTableCellElement> element(table.rows[row].cells[column].innerHTML);
...
async function plot_visualisation(visual : string[]) {
  var increment = 1;
