...
function getBlockedCells(grid_board: string[]) {

	let blocked_cells: number[] = [];
...
function getNodeCells(grid_board: string[]) {

	let node_cells: number[] = [];
...
function setEmptryGrid(x_dimension: number, y_dimension: number) {
...
function setWeights(x_dimension: number, y_dimension: number, index: number, is_blocked: boolean, grid: number[][]) {

	const row: number = Math.floor(index / x_dimension);

	const column: number = index - (x_dimension * row);

	let affected: number[] = [];

	var empty_grid: number[][] = grid;
...
function setGraph(x_dimension: number, y_dimension: number, blocked: number[], grid: number[][]) {

	let new_board = [];

	const blocked_cells: number[] = blocked;
...
function dijstras(graph: Object, node_cells: number[], blocked_cells: number[]) {

	let nodes: number[] = node_cells;
